...
if($settings['vfoods']['waimai']['ismc']) {
	if(!in_array($groupid,$settings['vfoods']['waimai']['mc'])) {
...
//强制跳出详情页
$isdefault = false;
$defaultshop = pdo_fetch("SELECT * FROM ".tablename('fm453_vfoods_category')." WHERE `uniacid` =:uniacid AND `iswaimai` =:iswaimai AND `isdefault`=:isdefault AND `enabled`=:enabled ORDER BY displayorder DESC , createtime ASC",array(':uniacid'=>$_W['uniacid'],':iswaimai'=>1,':isdefault'=>1,':enabled'=>1));	//全部店家
if($defaultshop) {
	$isdefault = true;
}
if($isdefault) {
