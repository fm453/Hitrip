...
if($settings['vfoods']['ziqu']['ismc']) {
	if(!in_array($groupid,$settings['vfoods']['ziqu']['mc'])) {
...
//强制跳出详情页
$isdefault = false;
$defaultshop = pdo_fetch("SELECT * FROM ".tablename('fm453_vfoods_category')." WHERE `uniacid` =:uniacid AND `isziqu` =:isziqu AND `isdefault`=:isdefault AND `enabled`=:enabled ORDER BY displayorder DESC , createtime ASC",array(':uniacid'=>$_W['uniacid'],':isziqu'=>1,':isdefault'=>1,':enabled'=>1));	//默认店家
if($defaultshop) {
	$isdefault = true;
}
if($isdefault) {
