...
// 	$_GPC['uid_openid'] ,传入进来的指定openid
if($_GPC['uid_openid']){
	$temp_uid=mc_openid2uid($_GPC['uid_openid']);
	// $temp_openid = $_GPC['uid_openid'];
	$article_id=pdo_fetchcolumn("SELECT id FROM " . tablename('fm453_site_article') . $condition ." AND a_tpl = 'member' AND goodadm = '".$_GPC['uid_openid']."' ",$params);
	if($article_id) {
		$url=fm_murl('article','detail','index',array('id'=>$article_id));
		header("Location: $url");
		exit;
	}else{
		$url = fm_murl('error','index','index',array('msg[title]'=>'未查到指定会员','msg[body]'=>'您查询的会员（'.$_GPC['keyword'].')('.$_GPC['uid_openid'].')资料暂未找到或未被公开……'));
		header("Location: ".$url);
		exit;
	}
}
...
//指定父分类时，仅显示子分类
if ($pcate>0) {
	$allcateurl=fm_murl('search', 'article', 'index', array()).$url_condition;//当前分类“全部文章”链接
	...
	$children = $allchidren;
	$allcateurl=fm_murl('search', 'article', 'index', array());//“全部分类”链接
	$haschild="";
...
if($ccate>0) {
	...
	$params[':ccate']=$ccid;
    $allcateurl=fm_murl('search', 'article', 'index', array("pcate" =>$pcate));//当前分类“全部文章”链接
...
/*————————分类排序的记录与处理——————————*/
$sorturl_default = fm_murl('search', 'article', 'index', array("a_tpl"=>$a_tpl,"keyword" => $keyword, "pcate" => $pcate, "ccate" => $ccate,"page"=>$pindex));
...
$_share['link']= fm_murl($do, $ac, 'index', array('keyword'=>$keyword,'page'=>$_GPC['page'],'isrecommand'=>$_GPC['isrecommand'],'isnew'=>$_GPC['isnew'],'ishot'=>$_GPC['ishot'],'istime'=>$_GPC['istime'],'pcate' =>$pcate, 'ccate' =>$ccate, 'a_tpl' =>$a_tpl,'isshare'=>1));
