...
$rule = pdo_fetchcolumn('SELECT rule FROM '.tablename('fm453_shopping_rules')." WHERE uniacid = :uniacid" , array(':uniacid' => $_W['uniacid']));
$id = pdo_fetchcolumn('SELECT id FROM '.tablename('fm453_shopping_member')." WHERE uniacid = :uniacid AND from_user = :from_user" , array(':uniacid' => $_W['uniacid'],':from_user' => $from_user));
