...
$from_shareid=intval($_GPC['shareid']);
if($from_shareid){
	$urladdons['shareid']=$from_shareid;
	$condition .= " AND shareid = :shareid";
	$params[':shareid']=$from_shareid;
}else{
	if($uid<=0) {
		message('请先选择关联会员',fm_wurl('crm','member','',array()),'info');
	}
	$urladdons['fromuser']=$fromuser;
	$openid = $fromuser;
	if(is_numeric($fromuser)) {
		//传入的是会员uid
...
if($operation=='display'){//此处接收来自产品列表中的产品ID参数并据此查询得出产品列表
	...
	//获取产品相关信息
	$ordergoods = pdo_fetch("SELECT * FROM " . tablename('fm453_shopping_goods') . "  WHERE  id= :id",array(':id'=>$gid));
...
elseif ($operation == 'export') {//此处接收来自产品列表中的产品ID参数并据此查询得出产品列表
	...
	if ($_GPC['export']) {
		...
		header("Content-Disposition:attachment; filename=".$shopname."订单列表-关联会员".$uid.".xls");
		echo $html;
