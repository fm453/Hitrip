...
fm_load()->fm_model('goodstpl'); //产品模型管理模块

$isorderrepaired = TRUE;
if($isorderrepaired !=TRUE) {
	fmMod_order_repair();
}
...
//开始操作管理
$shopname=$_FM['settings']['brands']['shopname'];
...
$condition .= ' AND ( `ordertype` = :ordertype  OR `ordertype` = " " )';
$params[':ordertype'] = 'biaozhun';
...
elseif($operation=='export') {
	...
	if ($_GPC['export']) {
		...
		header("Content-Disposition:attachment; filename=".$shopname."订单列表".$searchtime['start']."-".$searchtime['end'].".xls");
		echo $html;
		...
		unset($dologs);
		//给管理员发个微信消息
		include_once MODULE_ROOT.'/core/msgtpls/tpls/task.php';
		$noticeData = $tplnotice_data['task']['order']['export']['admin'];
		$msgResult = fmMod_notice_tpl($noticeData);
		if(!$msgResult) {
			include_once MODULE_ROOT.'/core/msgtpls/msg/order.php';
			$noticeData = $notice_data['order']['export']['admin'];
