...
elseif ($operation == 'update') {
	if($from_ac=='recyle' && $from_op=='display' && $update=='goods') {
		$sn=intval($_GPC['sn']);//将商品SN序号作为并行条件，避免前端错误传入商品ID
		$result=pdo_update('fm453_shopping_goods',array('deleted'=>0),array('id' => $id,'sn'=>$sn), 'AND');
		if($result){
		...
		if (in_array($type, array('isnew', 'ishot', 'isrecommand', 'isdiscount', 'isagent','hasoption','status','type'))) {
			$data = ($data==1?'0':'1');
			pdo_update("fm453_shopping_goods", array($type => $data), array("id" => $id));
			die(json_encode(array("result" => 1, "data" => $data)));
		}
	}
	die(json_encode(array("result" => 0)));
} elseif ($operation == 'delete') {
	if($from_ac=='list' && $from_op=='display' && $delete=='goods') {
		$sn=intval($_GPC['sn']);//将商品SN序号作为并行条件，避免前端错误传入商品ID
		$result=pdo_update('fm453_shopping_goods',array('deleted'=>1),array('id' => $id,'sn'=>$sn), 'AND');
		if($result){
