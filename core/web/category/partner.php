...
elseif ($operation == 'modify') {
	...
	if (!empty($id)) {
		...
		message('请先选择一款产品再操作！', 'referer', 'info');
	}
...
elseif ($operation == 'copy') {
	...
	if (!empty($id)) {
		$category = pdo_fetch("SELECT * FROM " . tablename('fm453_shopping_category') . " WHERE id = '{$id}'");
		if(!$category) {
			message('您要复制的产品无效，请重新选择！', 'referer', 'info');
		}
	} else {
		message('请先选择一款产品再操作！', 'referer', 'info');
	}
	...
	if (checksubmit('submit')) {
		...
		$data = array(
			...
			fmMod_log_record($platid,$_W['uniacid'],$_W['uid'],$fanid,$openid,'fm453_shopping_category',$id,'add',$dologs);
			unset($dologs);
			message('添加分类成功！', fm_wurl($do,$ac,'modify', array('id'=>$id,'psn'=>$psn)), 'success');
		}
	//包含的模板文件
