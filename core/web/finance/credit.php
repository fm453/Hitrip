...
if (!empty($category)) {
 ...
 if ($operation == 'display') {
		$condition = '';
		$sql = "SELECT * FROM ".tablename('fm453_shopping_credit_award')." as t1,".tablename('fm453_shopping_credit_request')."as t2 WHERE t1.id=t2.id AND t1.uniacid = '{$_W['uniacid']}' ORDER BY t2.createtime DESC";
...
else if ($operation == 'delete') { //删除兑换请求
		$id = intval($_GPC['id']);
		$row = pdo_fetch("SELECT id FROM ".tablename('fm453_shopping_credit_request')." WHERE id = :id", array(':id' => $id));
		if (empty($row)) {
				message('抱歉，编号为'.$id.'的兑换请求不存在或是已经被删除！');
		}
		pdo_delete('fm453_shopping_credit_request', array('id' => $id));
