...
elseif ($operation == 'modify')
{
	...
	if($_W['ispost']) {
		...
		$albumData = ['thumbs'=>'thumbs','scene'=>'scene','virtual'=>'virtual'];
		$Data = $albumData;
		if(!empty($Data)){
			foreach($Data as $k => $v){
				if(isset($_GPC[$k])){
		...
		//日期数据处理
		$dateData = ['opendate','jiaofangdate'];
		$Data = $dateData;
		if(!empty($Data)){
			foreach($Data as $k){
				if(isset($postData[$k])){
		...
		//整型数据处理
		$interData = ['status','deleted','open','jiaofang','rec','hot'];
		$Data = $interData;
		if(!empty($Data)){
			foreach($Data as $k){
				if(isset($postData[$k])){
...
elseif ($operation == 'delete')
{
	...
	$postData['deleted'] = 1;
		...
		//结果通知
		if ($isSuccess) {
			message('删除楼盘成功！',referer(), 'success');
		}
}