...
if ($operation == 'display') {
	//产品总数
	$sqlnum = 'SELECT count(id) FROM ' . tablename('fm453_shopping_goods') .$condition;
	$total = pdo_fetchcolumn($sqlnum, $params);
	//产品清单
	$sql = 'SELECT * FROM ' . tablename('fm453_shopping_goods') .$condition. $limit;
	$ds = pdo_fetchall($sql, $params);
	//结果列表分页
	$pager = pagination($total, $pindex, $psize);
	include $this->template($fm453style.$do.'/'.$ac.'/'.$operation.'/index');
}
elseif ($operation == 'search') {
	$sql = 'SELECT * FROM ' . tablename('fm453_shopping_goods') .$condition. $showorder;
	$ds = pdo_fetchall($sql, $params);
	foreach ($ds as &$value) {
		$value['thumb'] = tomedia($value['thumb']);
		$value['xsthumb'] = tomedia($value['xsthumb']);
	}
	include $this->template('modset/query');
}
