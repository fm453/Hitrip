...
if ($operation == 'display')
{
	...
	if($total==0){
		if(!$isSearching){
			message('还没有相应的模型数据，现在跳转至新增链接！', fm_wurl($do,$ac,'add',array()), 'info');
		}
	}
...
elseif ($operation == 'add')
{
	...
	if(checksubmit()){
		...
		//结果通知
		if ($isSuccess) {
			message('添加模型成功！',fm_wurl($do,$ac,'modify',$data), 'success');
		}
	}
...
elseif ($operation == 'modify')
{
	...
	if(checksubmit()){
		...
		//结果通知
		if ($isSuccess) {
			message('编辑模型成功！',fm_wurl($do,$ac,'modify',$data), 'success');
		}
	}
...
elseif ($operation == 'copy')
{
	...
	if(checksubmit()){
		...
		//结果通知
		if ($isSuccess) {
			message('复制模型成功！',fm_wurl($do,$ac,'modify',$data), 'success');
		}
	}
...
elseif ($operation == 'delete')
{
	...
	$postData['deleted'] = 1;
		...
		//结果通知
		if ($isSuccess) {
			message('删除模型成功！',referer(), 'success');
		}
}
