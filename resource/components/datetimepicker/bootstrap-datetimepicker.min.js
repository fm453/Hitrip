!function (c) {
    ...
    var e = function (i, h) {
        var k = this;
        this.element = c(i);
        this.container = h.container || "body";
        this.language = h.language || this.element.data("date-language") || "en";
        this.language = this.language in d ? this.language : "en";
        this.isRTL = d[this.language].rtl || false;
        this.formatType = h.formatType || this.element.data("format-type") || "standard";
        this.format = b.parseFormat(h.format || this.element.data("date-format") || d[this.language].format || b.getDefaultFormat(this.formatType, "input"), this.formatType);
        this.isInline = false;
        this.isVisible = false;
        this.isInput = this.element.is("input");
        this.fontAwesome = h.fontAwesome || this.element.data("font-awesome") || false;
        this.bootcssVer = h.bootcssVer || (this.isInput ? (this.element.is(".form-control") ? 3 : 2) : (this.bootcssVer = this.element.is(".input-group") ? 3 : 2));
        ...
        this.linkField = h.linkField || this.element.data("link-field") || false;
        this.linkFormat = b.parseFormat(h.linkFormat || this.element.data("link-format") || b.getDefaultFormat(this.formatType, "link"), this.formatType);
        this.minuteStep = h.minuteStep || this.element.data("minute-step") || 5;
        this.pickerPosition = h.pickerPosition || this.element.data("picker-position") || "bottom-right";
        this.showMeridian = h.showMeridian || this.element.data("show-meridian") || false;
        this.initialDate = h.initialDate || new Date();
        this.icons = {
        ...
        if ("formatViewType" in h) {
            this.formatViewType = h.formatViewType
        } else {
        ...
        if ("wheelViewModeNavigation" in h) {
            this.wheelViewModeNavigation = h.wheelViewModeNavigation
        } else {
        ...
        if ("wheelViewModeNavigationInverseDirection" in h) {
            this.wheelViewModeNavigationInverseDirection = h.wheelViewModeNavigationInverseDirection
        } else {
        ...
        if ("wheelViewModeNavigationDelay" in h) {
            this.wheelViewModeNavigationDelay = h.wheelViewModeNavigationDelay
        } else {
        ...
        if ("startView" in h) {
            this.startViewMode = h.startView
        } else {
        ...
        if ("forceParse" in h) {
            this.forceParse = h.forceParse
        } else {
            if ("dateForceParse" in this.element.data()) {
                this.forceParse = this.element.data("date-force-parse")
            }
        }
        var j = this.bootcssVer === 3 ? b.templateV3 : b.template;
        while (j.indexOf("{iconType}") !== -1) {
            j = j.replace("{iconType}", this.icontype)
        }
        while (j.indexOf("{leftArrow}") !== -1) {
            j = j.replace("{leftArrow}", this.icons.leftArrow)
        }
        while (j.indexOf("{rightArrow}") !== -1) {
            j = j.replace("{rightArrow}", this.icons.rightArrow)
        }
        this.picker = c(j).appendTo(this.isInline ? this.element : this.container).on({
        ...
        if (this.isRTL) {
            ...
            var g = this.bootcssVer === 3 ? ".prev span, .next span" : ".prev i, .next i";
            this.picker.find(g).toggleClass(this.icons.leftArrow + " " + this.icons.rightArrow)
        }
        c(document).on("mousedown", function (l) {
            if (c(l.target).closest(".datetimepicker").length === 0) {
                k.hide()
            }
        });
        this.autoclose = false;
        if ("autoclose" in h) {
            this.autoclose = h.autoclose
        } else {
        ...
        if ("keyboardNavigation" in h) {
            this.keyboardNavigation = h.keyboardNavigation
        } else {
            if ("dateKeyboardNavigation" in this.element.data()) {
                this.keyboardNavigation = this.element.data("date-keyboard-navigation")
            }
        }
        this.todayBtn = (h.todayBtn || this.element.data("date-today-btn") || false);
        this.todayHighlight = (h.todayHighlight || this.element.data("date-today-highlight") || false);
        this.weekStart = ((h.weekStart || this.element.data("date-weekstart") || d[this.language].weekStart || 0) % 7);
        this.weekEnd = ((this.weekStart + 6) % 7);
        this.startDate = -Infinity;
        this.endDate = Infinity;
        this.daysOfWeekDisabled = [];
        this.setStartDate(h.startDate || this.element.data("date-startdate"));
        this.setEndDate(h.endDate || this.element.data("date-enddate"));
        this.setDaysOfWeekDisabled(h.daysOfWeekDisabled || this.element.data("date-days-of-week-disabled"));
        this.setMinutesDisabled(h.minutesDisabled || this.element.data("date-minute-disabled"));
        this.setHoursDisabled(h.hoursDisabled || this.element.data("date-hour-disabled"));
        this.fillDow();
        this.fillMonths();
        this.update();
        this.showMode();
    ...
    e.prototype = {
        constructor: e, _events: [], _attachEvents: function () {
            ...
            this.update();
            this.updateNavArrows()
        }, setMinutesDisabled: function (g) {
            this.minutesDisabled = g || [];
            if (!c.isArray(this.minutesDisabled)) {
                this.minutesDisabled = this.minutesDisabled.split(/,\s*/)
            }
            this.minutesDisabled = c.map(this.minutesDisabled, function (h) {
                return parseInt(h, 10)
            });
            this.update();
            this.updateNavArrows()
        }, setHoursDisabled: function (g) {
            this.hoursDisabled = g || [];
            if (!c.isArray(this.hoursDisabled)) {
                this.hoursDisabled = this.hoursDisabled.split(/,\s*/)
            }
            this.hoursDisabled = c.map(this.hoursDisabled, function (h) {
                return parseInt(h, 10)
            });
            this.update();
            this.updateNavArrows()
            ...
            var G = new Date(this.viewDate), s = G.getUTCFullYear(), H = G.getUTCMonth(), l = G.getUTCDate(),
                B = G.getUTCHours(), w = G.getUTCMinutes(),
                ...
                n = this.endDate !== Infinity ? this.endDate.getUTCFullYear() : Infinity,
                y = this.endDate !== Infinity ? this.endDate.getUTCMonth() + 1 : Infinity,
                p = (new f(this.date.getUTCFullYear(), this.date.getUTCMonth(), this.date.getUTCDate())).valueOf(),
                F = new Date();
            this.picker.find(".datetimepicker-days thead th:eq(1)").text(d[this.language].months[H] + " " + s);
            if (this.formatViewType == "time") {
                var D = B % 12 ? B % 12 : 12;
                var j = (D < 10 ? "0" : "") + D;
                var o = (w < 10 ? "0" : "") + w;
                var J = d[this.language].meridiem[B < 12 ? 0 : 1];
                this.picker.find(".datetimepicker-hours thead th:eq(1)").text(j + ":" + o + " " + (J ? J.toUpperCase() : ""));
                this.picker.find(".datetimepicker-minutes thead th:eq(1)").text(j + ":" + o + " " + (J ? J.toUpperCase() : ""))
            } else {
                this.picker.find(".datetimepicker-hours thead th:eq(1)").text(l + " " + d[this.language].months[H] + " " + s);
                this.picker.find(".datetimepicker-minutes thead th:eq(1)").text(l + " " + d[this.language].months[H] + " " + s)
            }
            this.picker.find("tfoot th.today").text(d[this.language].today).toggle(this.todayBtn !== false);
            this.updateNavArrows();
            this.fillMonths();
            var K = f(s, H - 1, 28, 0, 0, 0, 0), A = b.getDaysInMonth(K.getUTCFullYear(), K.getUTCMonth());
            K.setUTCDate(A);
            K.setUTCDate(A - (K.getUTCDay() - this.weekStart + 7) % 7);
            var g = new Date(K);
            ...
            while (K.valueOf() < g) {
                ...
                t = "";
                if (K.getUTCFullYear() < s || (K.getUTCFullYear() == s && K.getUTCMonth() < H)) {
                    t += " old"
                } else {
                    if (K.getUTCFullYear() > s || (K.getUTCFullYear() == s && K.getUTCMonth() > H)) {
                        t += " new"
                ...
                if (this.todayHighlight && K.getUTCFullYear() == F.getFullYear() && K.getUTCMonth() == F.getMonth() && K.getUTCDate() == F.getDate()) {
                    t += " today"
                }
                if (K.valueOf() == p) {
                    t += " active"
                }
                if ((K.valueOf() + 86400000) <= this.startDate || K.valueOf() > this.endDate || c.inArray(K.getUTCDay(), this.daysOfWeekDisabled) !== -1) {
                    t += " disabled"
                }
                q.push('<td class="day' + t + '">' + K.getUTCDate() + "</td>");
            ...
            for (var z = 0; z < 24; z++) {
                ...
                if (this.showMeridian && d[this.language].meridiem.length == 2) {
                    E = (z < 12 ? d[this.language].meridiem[0] : d[this.language].meridiem[1]);
                    if (E != r) {
                        if (r != "") {
                            q.push("</fieldset>")
                        }
                        q.push('<fieldset class="hour"><legend>' + E.toUpperCase() + "</legend>")
                    }
                    r = E;
                    u = (z % 12 ? z % 12 : 12);
            ...
            for (var z = 0; z < 60; z += this.minuteStep) {
                ...
                if (this.showMeridian && d[this.language].meridiem.length == 2) {
                    E = (B < 12 ? d[this.language].meridiem[0] : d[this.language].meridiem[1]);
                    if (E != r) {
                        if (r != "") {
                            q.push("</fieldset>")
                        }
                        q.push('<fieldset class="minute"><legend>' + E.toUpperCase() + "</legend>")
                    }
                    r = E;
                    u = (B % 12 ? B % 12 : 12);
            ...
            var m = this.picker.find(".datetimepicker-months").find("th:eq(1)").text(s).end().find("span").removeClass("active");
            if (L == s) {
                m.eq(this.date.getUTCMonth() + 2).addClass("active")
            }
            if (s < x || s > n) {
                m.addClass("disabled")
            }
            if (s == x) {
                m.slice(0, C + 1).addClass("disabled")
            }
            if (s == n) {
                m.slice(y).addClass("disabled")
            }
            q = "";
            ...
            s -= 1;
            for (var z = -1; z < 11; z++) {
                q += '<span class="year' + (z == -1 || z == 10 ? " old" : "") + (L == s ? " active" : "") + (s < x || s > n ? " disabled" : "") + '">' + s + "</span>";
                s += 1
            }
            I.html(q);
            this.place()
        }, updateNavArrows: function () {
            var k = new Date(this.viewDate), i = k.getUTCFullYear(), j = k.getUTCMonth(), h = k.getUTCDate(),
                g = k.getUTCHours();
            ...
            if (l.is("." + this.icontype)) {
                l = c(l).parent().closest("span, td, th, legend")
            }
            if (l.length == 1) {
                if (l.is(".disabled")) {
                ...
                switch (l[0].nodeName.toLowerCase()) {
                    case"th":
                        switch (l[0].className) {
                            ...
                            case"next":
                                ...
                                this.fill();
                                this.element.trigger({
                                    type: l[0].className + ":" + this.convertViewModeText(this.viewMode),
                                    date: this.viewDate,
                                    startDate: this.startDate,
                                    endDate: this.endDate
                                });
                    ...
                    case"span":
                        if (!l.is(".disabled")) {
                            var n = this.viewDate.getUTCFullYear(), m = this.viewDate.getUTCMonth(),
                                o = this.viewDate.getUTCDate(), p = this.viewDate.getUTCHours(),
                                i = this.viewDate.getUTCMinutes(), q = this.viewDate.getUTCSeconds();
                            if (l.is(".month")) {
                    ...
                    case"td":
                        if (l.is(".day") && !l.is(".disabled")) {
                            ...
                            var n = this.viewDate.getUTCFullYear(), m = this.viewDate.getUTCMonth(),
                                p = this.viewDate.getUTCHours(), i = this.viewDate.getUTCMinutes(),
    ...
    var b = {
        ...
        parseDate: function (l, u, o, r) {
            ...
            var m = l && l.match(this.nonpunctuation) || [], l = new Date(0, 0, 0, 0, 0, 0, 0), q = {},
